  ...
  const performSave = async () => {
    try {
      ...
      containerIds.forEach((id) => {
        ...
        if (el) {
          s[id] = { t: el.scrollTop || 0, l: el.scrollLeft || 0 };
        }
      });

      const modalBox = document.querySelector(".modal-box");
      if (modalBox) {
        s["modal-box"] = { t: modalBox.scrollTop || 0, l: modalBox.scrollLeft || 0 };
        ["detailsWrapper", "transformWrapper", "bkmWrapper", "rangeWrapper", "gsiWrapper"].forEach((wId) => {
          const wEl = document.getElementById(wId);
          if (wEl && wEl.style.display !== "none") {
            s["mb_" + wId] = { t: modalBox.scrollTop || 0, l: modalBox.scrollLeft || 0 };
            if (wId === "bkmWrapper" && typeof window.currentBkmTab !== "undefined") {
              s["mb_bkmWrapper_" + window.currentBkmTab] = { t: modalBox.scrollTop || 0, l: modalBox.scrollLeft || 0 };
            }
          }
        });
      }
      ...
      filterWrappers.forEach((el) => {
        if (el.id) {
          s[el.id] = { t: el.scrollTop || 0, l: el.scrollLeft || 0 };
        }
      });
      ...
      fontWrappers.forEach((el) => {
        ...
        if (fcat) {
          s[fcat] = { t: el.scrollTop || 0, l: el.scrollLeft || 0 };
        }
      });
      ...
      const getUIObj = (id) => {
        const el = document.getElementById(id);
        if (!el) return null;
        return { c: el.className || "", d: el.style.display || "" };
      };
...
function attachAutoSaveListeners() {
  document.addEventListener("input", () => window.AS(), { passive: true });
  document.addEventListener("change", () => window.AS(), { passive: true });
  document.addEventListener("click", () => window.AS(), { passive: true });
