function buildCardHTML(e, t, a) {
    ...
    if (!a) {
        i += '<button onclick="dLPNG(' + t + ')" style="background:none; border:none; color:#fff; cursor:pointer; text-align:left; padding:5px 10px; font-size:0.8rem; border-radius:4px;">Export PNG</button>';
    }
...
function generateExportContent(e, t) {
    if ("json" === t || "txt" === t) {
        ...
        if (0 === Object.keys(a.SINGLE).length) delete a.SINGLE;
        if (0 === Object.keys(a.COMBINED).length) delete a.COMBINED;
        if ("json" === t) return JSON.stringify(a, null, 2);
        var r = "";
        for (var k1 in a) {
            for (var k2 in a[k1]) {
                r += "\n\n[ " + k1 + " - " + k2 + " ]\n";
                r += "=".repeat(k1.length + k2.length + 7) + "\n\n";
                a[k1][k2].forEach(function(e) {
                    var t = "=== " + e.CHARACTER + " ===\n";
    ...
    if ("html" === t) {
        var cards = e.map(function(e, t) {
            return buildCardHTML(e, t, false);
        }).join("");
        return getHTMLHeader() + cards + getHTMLFooter(false);
    }
}